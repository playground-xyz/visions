 ...
 * @ignore
 */
module.exports = function doJoins(res, models, originalModel, modifiedModel, calcCount, manualCount) {
  ...
  if (calcCount) {
    if (res.length > 0) {
      joined.count = parseInt(res[0].full_count, 10);
    } else {
      joined.count = 0;
    }

    // When we are filtering by a non-core entity, the count has to be manual. This
    // is fine as limit and skip aren't allowed to used at the same time as this
    // type of filtering
    if (manualCount) {
      joined.count = joined.length;
    }
  }
