...
class PreparedQuery {
  ...
  constructor(modelId, models, knex) {
    ...
    this.whereClauseAfterJoin = null;
  ...
  /**
   * Define a restriction of a non-core entity on the result set.
   *
   * @param {Object} clause - An object specifying the
   *     predicate that all results must satisfy. A predicate consists
   *     of a `key` (the column name) and a `value`.
   */
  whereAfterJoin(clause) {
    if (!clause) {
      return this;
    }

    if (!(clause instanceof Object)) {
      throw new Error('whereAfterJoin must be an object');
    }

    if (
      clause.primaryJoin === undefined ||
      clause.secondaryJoin === undefined ||
      clause.value === undefined
    ) {
      throw new Error('whereAfterJoin must have properties: primaryJoin, secondaryJoin, value');
    }

    this.whereClauseAfterJoin = clause;
