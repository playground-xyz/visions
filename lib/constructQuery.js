...
function addAssociationSelects(assoc, model, models) {
  ...
  if (!assocModel) {
    throw new Error(`Invalid mapId: ${assoc.mapId}`);
  }
 ...
 * @ignore
 */
function constructSelects(models, model, calcCount) {
  ...
  const statements = [].concat.apply([], allSelects);
  if (calcCount) {
    statements.push('full_count AS full_count');
  }
  return statements;
}
 ...
 * @ignore
 */
function constructSubQuery(qb, knex, viewId, limit, skip,
  sortAttr, sortDir, whereClauses, calcCount, whereClauseAfterJoin) {

  qb = qb.select(`${viewId}.*`).from(viewId);
  if (whereClauseAfterJoin) {
    const { primaryJoin, secondaryJoin, value } = whereClauseAfterJoin;
    if (primaryJoin === 'campaign_view_main') {
      qb.leftJoin(`${primaryJoin}`, `${primaryJoin}.id`, `${viewId}.campaign`);
    } else {
      qb.leftJoin(`${primaryJoin}`, `${primaryJoin}.id`, `${viewId}.${primaryJoin}`);
    }
    qb.where(`${primaryJoin}.${secondaryJoin}`, value);
  }

  if (calcCount) {
    qb.select(knex.raw('count(*) OVER() AS full_count'));
  }
  if (limit) {
    qb = qb.limit(limit);
  }
 ...
 */
function constructQuery(models, model, knex, whereClauses, sortAttr,
  sortDir, limit, skip, calcCount, whereClauseAfterJoin) {
  ...
  let queryObj = knex
    .with(`${model.viewId}-core`, qb =>
      constructSubQuery(
        qb, knex, model.viewId, limit, skip, sortAttr, sortDir, coreWClauses, calcCount, whereClauseAfterJoin
      )
    );
  ...
  // be returned
  queryObj = queryObj
    .select(constructSelects(models, model, calcCount))
  ...
  const collJoinTableParams = model.collections
    ...
    .map(coll => [
      ...
      helpers.createCollectionJoinTableParameters(
        coll.mapId,
        coll.joinTable,
        coll.viewId
      )
  ...
  const joinParams = assocJoinParams
    .concat(collJoinParams)
    .concat(flatten(collJoinTableParams))
    .filter(p => !!p);
  ...
  if (whereClauseAfterJoin) {
    const { primaryJoin, secondaryJoin, value } = whereClauseAfterJoin;
    const table = `${primaryJoin}.${secondaryJoin}`;
    queryObj = queryObj.where(table, value);
  }
