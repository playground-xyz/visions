...
function constructSubQuery(qb, viewId, limit, skip, sortAttr, sortDir) {
...
function constructQuery(models, model, knex, whereClauses, sortAttr, sortDir, limit, skip) {
  ...
  let queryObj = knex
      ...
      .with(`${model.viewId}-core`, qb =>
            constructSubQuery(qb, model.viewId, limit, skip, sortAttr, sortDir))
  ...
  queryObj = helpers.applyWhereClauses(queryObj,
      whereClauses.map(clause => preProcessWhereClause(clause, models, model))
  );
